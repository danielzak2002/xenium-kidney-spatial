...
*Methods robustness note.* The cLN sections are multi-core (8–43 cores/slide separated by mm gaps); the per-section Delaunay graph therefore spans cores. This was checked: pruning all cross-core edges (>50 µm) shifts the plasma×myeloid z by <0.3 on every slide (e.g. SP21_213 16.97→17.15, control 5.48→5.49), because the permutation null uses the same graph — so the gap-spanning edges do not distort the z, and the per-slide values stand.
